...
- активация камеры через `ARM CAM` (`getUserMedia`)
- диагностика камеры:
  - `CAM` статус (`OFF/LIVE/DENY/N/A`)
  - `CAM MOTION`
  - `CAM LIGHT`
  - шкала движения + удержание пика
  - окно превью
- опциональная модуляция от камеры:
  - `CAM MOD` мягко влияет на threshold, route weight и fault intensity
...
8. (Опционально) включи камеру:
   - нажми `ARM CAM`
   - разреши доступ
   - поставь `EXT SOURCE = CAM MOTION`
   - включи `CAM MOD` при необходимости
9. Отслеживай реакцию через телеметрию и scope.
10. Сохраняй состояния в snapshot-слоты.
...
- камерный тракт пока ориентирован на анализ движения/яркости, без компьютерного зрения более высокого уровня
